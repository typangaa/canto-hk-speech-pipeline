...
| `asr.agreement` | `asr_results` | `asr_agreement` | cross-model char-overlap agreement score (punctuation/digit-normalized since 2026-07-13, corpus-wide backfilled 2026-07-15), computed over the 2 active models only; still captures `canto_ft_confidence` from historical rows where present, purely informational since 2026-07-15 — `tier.assign`'s `auto_gold` gate now uses `filters.dnsmos` instead, see the tier note below |
...
| `quality_tier.assign` | `tiers`(gold/auto_gold scope only)+`filters`+`labels_music`+`labels_overlap` | `quality_tiers` | **T13, added 2026-07-16**: the label-framework A/B **acoustic-cleanliness** tier from `docs/LABEL_FRAMEWORK_SPEC.md` §10 — Tier A (pretrain, permissive) = the entire gold+auto_gold scope; Tier B (clean fine-tune, strict) = a subset gated on `dnsmos≥3.7 AND music_prob<0.10 AND overlap_ratio<0.05`. **Different axis from `tier.assign`** — see `pipeline/nodes/quality_tier.py`'s module docstring. Scoped to gold+auto_gold only (not the full manifest-eligible pool) per owner decision, since canto-tts's imminent training run only wants that verification-confidence band. `manifest.build`/`manifest.export`'s `--min-quality-tier {A,B}` consumes it (combinable with `--min-tier`) — see `pending_task.md` T13. |
| `calibrate.sample` | `asr_agreement`+`filters`+`tiers` | `calibration_review` | queues a random sample (optionally scoped by `tier`/`min_agreement`) for human text-verification review via `pipe calibrate serve`'s browser UI; a `'verified'` decision flips `asr_agreement.text_verified` + `tiers.tier='gold'`. **Offline mode (2026-07-13)**: `calibrate serve` never blocks on the catalog anymore — every decision is buffered to a local JSONL (`metadata/calibration_pending_decisions.jsonl`) instead of writing straight to DuckDB, and reads fall back to a JSON snapshot (`pipe calibrate export-snapshot`, `metadata/calibration_offline_queue.json`) when the writer lock is held by a long batch node. Run `pipe calibrate flush-pending` once the writer is free to replay buffered decisions into the catalog (safe to re-run). See `pipeline/tools/calibrate_server.py`'s module docstring and DECISIONS.md 2026-07-13. |
...
(added `auto_gold` 2026-07-10, thresholds raised + `bronze` added 2026-07-11, gate
rebuilt 2026-07-15, DECISIONS.md): `gold` (human text-verified via `pipe calibrate
serve`), `auto_gold` (**not** human-verified — a statistical-confidence tier: ASR
agreement ≥ 0.92 AND `filters.dnsmos` ≥ 3.5, sample-QA'd rather than exhaustively
reviewed — never treat as equivalent to `text_verified`. Rebuilt 2026-07-15 after
`canto_ft`'s 2026-07-13 retirement broke the original `canto_ft_confidence`-based gate
(always `NULL` for new segments, failing closed) — `asr_agreement.agreement` was
corpus-wide backfilled using the already-shipped punctuation/digit normalization fix
(char_agreement()) with canto_ft excluded, then the confidence gate was replaced with
`filters.dnsmos` as a zero-new-compute, non-ASR third trust signal (industry precedent:
GigaSpeech 2 / Emilia-Pipe-style pipelines layer LID/DNSMOS on top of ASR agreement
rather than trusting agreement alone). Provisional pending T1 pilot-QA ground-truth
validation — see `pending_task.md` T16 and DECISIONS.md 2026-07-15), `silver` (ASR
agreement ≥ 0.85, unchanged), `bronze` (ASR agreement ≥ 0.70, unchanged — the
...
`docs/LABEL_FRAMEWORK_SPEC.md` §10 separately defines an **A/B TTS-quality** tier
(pretrain vs. clean) built from the label store — built 2026-07-16 (T13) as the
`quality_tier.assign` node / `quality_tiers` table, scoped to gold+auto_gold only (see the
node table above). Do not conflate the two when reading/writing either one. See
`docs/FINDINGS_ASR_AGREEMENT_THRESHOLDS.md` for the
