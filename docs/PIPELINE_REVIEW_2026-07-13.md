...
| 20 | Medium | `char_agreement()`(`pipeline/nodes/asr.py:347`)用 raw `difflib.SequenceMatcher`,**零 text normalization** — AR model(qwen3_asr)靠 context 補標點、CTC model(sense_voice)天生唔識標點,兩者比較會被 punctuation mismatch **系統性壓低 agreement**(§5 調研確認呢個係 AR-vs-CTC 對比嘅第一大 pitfall)。喺 3-model 年代呢個 bias 被攤薄;而家 agreement 係唯一 trust signal,bias 直接影響 tier 分佈 | T16 嘅一部份:agreement 計算前加 punctuation-strip + 數字 normalization(對 stripped string 計 overlap;原文照存)。**必須喺 T16 分佈分析之前做**,否則分析嘅分佈本身就係 biased |
