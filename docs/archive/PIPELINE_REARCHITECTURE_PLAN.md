# Pipeline Re-Architecture & Storage Migration Plan (archived)

> **狀態**:ARCHIVED 2026-07-19 —— **§3 Phase A + Phase B storage migration 已完成
> (2026-07-02)**,Drive1/2/4 ext4 化(Drive2/4)+ 遷移(Drive1)+ Drive3 reformat 全部做完,
> 見底部「§3 執行結果」。**§4 pipeline re-architecture 願景(decode-once/orchestrator/
> DuckDB)已經全部落地實現**(P0–P5 done)—— 見
> `docs/REARCHITECTURE_IMPLEMENTATION_PLAN.md`(具體 implementation plan + milestone
> 狀態,權威來源)。呢份係嗰份 implementation plan 嘅**上游 vision 文件**,而家淨係
> history reference,唔再更新。
> **先讀(當時)**:`docs/archive/PIPELINE_SPEC.md`、`LABEL_FRAMEWORK_SPEC.md`、`KNOWN_ISSUES.md`
