# Pipeline Specification — canto-hk-speech-pipeline (archived, legacy stage numbering)

> **狀態**:ARCHIVED 2026-07-19 —— 呢份文件成篇圍繞已經退役嘅 10-stage flat script chain
> (`01_discover.py` … `10_report.py`)寫,`scripts/` 而家已經清空,全部功能已經 port 咗做
> `pipeline/nodes/*.py` DAG node。**想睇現行 per-node implementation detail?** 讀對應 node
> file 嘅 module docstring(見 `CLAUDE.md` node table)。文件底下嘅演算法邏輯(VAD 參數、
> diarization 流程、SNR estimation method 等)仍然有歷史參考價值,原封保留唔改。
>
> Detailed implementation guide for each pipeline stage (legacy). Read alongside
> `CLAUDE.md` (overview + constraints) and `docs/KNOWN_ISSUES.md`.
